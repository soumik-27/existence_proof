...
export const CONTRACT_ADDRESS =
  "CB44YWEAMXJBNCMUSQKTLZDMHETIT7EPAR2WGE2T36DU5FN5MCKIH7D3";
...
export function toScValBytes(value: string): xdr.ScVal {
  // Convert base64 string to BytesN<32> ScVal
  return nativeToScVal(Buffer.from(value, "base64"), { type: "bytes" });
}

// ============================================================
// Proof of Existence — Contract Methods
// ============================================================

/**
 * Register a document hash on-chain.
 * ANYONE can call this (permissionless).
 * Calls: register(hash: BytesN<32>, submitter: Address, description: String)
 */
export async function registerDocument(
  ...
  return callContract(
    "register",
    [toScValBytes(hash), toScValAddress(caller), toScValString(description)],
    caller,
    true
  );
}
 ...
 * Returns registration info if found, null otherwise.
 * Calls: verify(hash: BytesN<32>) -> Option<Registration>
 */
 ...
 * Calls: get_total_count() -> u32
 */
export async function getTotalCount(caller?: string) {
  return readContract(
    "get_total_count",
    [],
    caller
...
/**
 * Get registered documents with pagination.
 * Calls: get_registrations(start: u32, end: u32) -> Vec<Registration>
 */
export async function getRegistrations(
  ...
  return readContract(
    "get_registrations",
    [toScValU32(start), toScValU32(end)],
    caller
  );
}
